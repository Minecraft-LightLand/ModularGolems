...
public class MechBodyModifier extends GolemModifier {
	...
	public void onAiStep(AbstractGolemEntity<?, ?> golem, int level) {
		...
		var mobileIns = golem.getEffect(CreateCompatRegistry.EFF_MOBILE);
		var forceIns = golem.getEffect(CreateCompatRegistry.EFF_FORCE);
		int mobileTime = 0;
		int forceTime = 0;
		...
		if (mobile > 0 && mobileTime < threshold ||
			...
			fuel.shrink(1);
			if (mobile > 0) golem.addEffect(new MobEffectInstance(CreateCompatRegistry.EFF_MOBILE,
					mobileTime + time, mobile - 1));
			if (force > 0) golem.addEffect(new MobEffectInstance(CreateCompatRegistry.EFF_FORCE,
