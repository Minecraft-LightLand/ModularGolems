...
import static dev.xkmc.l2core.serial.advancements.CriterionBuilder.Provider.*;
...
public class MGAdvGen {
	...
	public static void genAdvancements(RegistrateAdvancementProvider pvd) {
		...
		var arm = root
				.create("start", GolemItems.GOLEM_BODY.get(), helper.item(MGTagGen.GOLEM_PARTS),
						"The Beginning of Everything", "Craft a Golem Template and use Stone Cutter to cut it into a golem part")
				.create("apply", GolemItems.GOLEM_BODY.get().withMaterial(ModularGolems.loc("iron")),
						one(PartCraftTrigger.ins().build()),
						"Heavy Metal Piece", "Apply metal ingots onto golem parts in an anvil.");
		arm.create("apply_sculk", GolemItems.GOLEM_BODY.get().withMaterial(ModularGolems.loc("sculk")),
				one(PartCraftTrigger.withMat(ModularGolems.loc("sculk")).build()),
		...
		arm.create("humanoid", GolemItems.HOLDER_HUMANOID.get().withUniformMaterial(
				...
				.create("oops", GolemItems.HOLDER_HUMANOID.get().withUniformMaterial(
						...
						player(GolemTriggers.BREAK.get()),
		...
		upgrade.create("recycle", GolemItems.RECYCLE.get(),
				one(UpgradeApplyTrigger.withUpgrade(GolemItems.RECYCLE.get()).build()),
		...
		upgrade.create("sponge", GolemItems.SPONGE.get(),
						one(UpgradeApplyTrigger.withUpgrade(GolemItems.SPONGE.get()).build()),
						"Waterlogged Golem", "Apply a sponge upgrade on a golem").type(AdvancementType.GOAL)
				.create("kill_creeper", Items.TNT,
						one(GolemKillTrigger.byType(EntityType.CREEPER).build()),
