...
public class RetrivedGolemPage extends GolemInfoScreen {

	protected RetrivedGolemPage(Component title) {
		super(title);
	}

	@Override
	public List<Pair<UUID, GolemTracker.TrackedData>> getData() {
		return getData(e -> e.status == GolemTracker.Status.RETRIEVED && e.target != null);
	}
	...
	public void init() {
		...
		new TabManager<>(this, new TrackerGroup(GolemTabRegistry.TRACKERS))
				.init(this::addRenderableWidget, GolemTabRegistry.TRACKER_RETRIEVE.get());
	}
