...
public class GolemUpgradeItemHandler implements IItemHandlerModifiable {
	...
	private GolemUpgrade upgrades = null;
	...
	public ItemStack appendUpgrade(UpgradeItem upgrade) {
		...
		copy.add(upgrade);
		int remaining = holderItem.getRemaining(materials, new GolemUpgrade(upgrades.extraSlot(), copy));
	...
	public ItemStack removeUpgrade(IUpgradeItem upgrade) {
		var list = new ArrayList<>(upgrades.upgrades());
		if (!list.remove(upgrade.asItem())) return ItemStack.EMPTY;
		if (holderItem.getRemaining(materials, new GolemUpgrade(upgrades.extraSlot(), list)) < 0)
			return ItemStack.EMPTY;
		var ans = golem.copy();
		ans.set(GolemItems.UPGRADE, new GolemUpgrade(upgrades.extraSlot(), list));
		return ans;
	}
	...
	public void setStackInSlot(int slot, @NotNull ItemStack stack) {
		...
		boolean match = ItemStack.isSameItemSameComponents(old, stack);
