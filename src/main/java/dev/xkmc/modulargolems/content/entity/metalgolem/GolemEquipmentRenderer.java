...
public class GolemEquipmentRenderer extends RenderLayer<MetalGolemEntity, MetalGolemModel> {
	...
	protected void renderBeacon(PoseStack pose, MultiBufferSource source, float pTick, long gameTick, float height) {
		...
		addVertex(p, buffer, 1.0F, 1.0F, 1.0F, 1.0F, 0, 0, width, 0, 0);
		addVertex(p, buffer, 1.0F, 1.0F, 1.0F, 1.0F, 0, width, 0, 0, 1);
		addVertex(p, buffer, 1.0F, 1.0F, 1.0F, 1.0F, 0, 0, -width, 1, 1);
		addVertex(p, buffer, 1.0F, 1.0F, 1.0F, 1.0F, 0, -width, 0, 1, 0);
		pose.popPose();
	}

	final ResourceLocation BEAM_LOCATION = ResourceLocation.withDefaultNamespace("textures/entity/beacon_beam.png");
	...
	private static void renderPart(PoseStack pose, VertexConsumer buffer, float r, float g, float b, float a, int start, int end, float p_112164_, float p_112165_, float p_112166_, float p_112167_, float p_112168_, float p_112169_, float p_112170_, float p_112171_, float u1, float u2, float v1, float v2) {
		PoseStack.Pose p = pose.last();
		renderQuad(p, buffer, r, g, b, a, start, end, p_112164_, p_112165_, p_112166_, p_112167_, u1, u2, v1, v2);
		renderQuad(p, buffer, r, g, b, a, start, end, p_112170_, p_112171_, p_112168_, p_112169_, u1, u2, v1, v2);
		renderQuad(p, buffer, r, g, b, a, start, end, p_112166_, p_112167_, p_112170_, p_112171_, u1, u2, v1, v2);
		renderQuad(p, buffer, r, g, b, a, start, end, p_112168_, p_112169_, p_112164_, p_112165_, u1, u2, v1, v2);
	}
	...
	private static void renderQuad(PoseStack.Pose pose, VertexConsumer buffer, float r, float g, float b, float a, int y1, int y2, float x1, float z1, float x2, float z2, float u1, float u2, float v1, float v2) {
		addVertex(pose, buffer, r, g, b, a, y2, x1, z1, u2, v1);
		addVertex(pose, buffer, r, g, b, a, y1, x1, z1, u2, v2);
		addVertex(pose, buffer, r, g, b, a, y1, x2, z2, u1, v2);
		addVertex(pose, buffer, r, g, b, a, y2, x2, z2, u1, v1);
	}
	...
	private static void addVertex(PoseStack.Pose pose, VertexConsumer buffer, float r, float g, float b, float a, int y, float x, float z, float u, float v) {
		buffer.addVertex(pose, x, (float) y, z).setColor(r, g, b, a).setUv(u, v)
