...
public abstract class AbstractGolemRenderer<
	...
	protected RenderType getRenderType(T entity, boolean b1, boolean b2, boolean b3) {
		...
		pose.pushPose();
		for (P p : list.get()) {
			renderPart(p, entity, b1, b2, b3);
		}
		pose.popPose();
		return null;
	}

	private void renderPart(P type, T entity, boolean b1, boolean b2, boolean b3) {
		...
		if (rendertype != null) {
			...
			this.model.renderToBufferInternal(type, hand.stack(), vertexconsumer, hand.i(), i);
		}
	}
