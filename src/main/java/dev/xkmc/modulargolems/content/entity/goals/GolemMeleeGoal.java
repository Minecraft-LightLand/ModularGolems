...
public class GolemMeleeGoal extends MeleeAttackGoal implements IMeleeGoal {
	...
	public boolean canContinueToUse() {
		...
		} else if (!golem.isWithinHome(livingentity.blockPosition())) {
			return false;
		} else {
	...
	public void stop() {
		...
		if (le != null && !EntitySelector.NO_CREATIVE_OR_SPECTATOR.test(le)) {
	...
	private boolean isInFluidType() {
		return golem.isInWater() || golem.isInLava();
	}
	...
	protected void repath(LivingEntity target, double dist) {
		...
		if (activeRepathTime > golem.tickCount && golem.getNavigation().isDone() && (golem.isInWater() || golem.onGround())) {
